...
def get_source_clusters(n_src, n_stim, rho_src):
    """
...
def compute_density_matrix(A):
    """
    Returns the connection density of the matrix.
    :param A:
    :return:
    """
...
class ParameterSet(dict):
    def __init__(self, d):
        ...
        self.m = d['m']
        self.sigma = d['sigma']
        self.n_stim = d['n_stim']
        self.wE = d['wE']
        self.delta = d['delta']
        self.N = d['N']


def clustered_connections(n_neurons, n_clusters, density, modularity, rho0=None, delta=0.):
    """
    Generate a modular adjacency matrix.

    :param n_neurons: number of pre- and post-synaptic neurons (assumes equal)
    ...
    :param rho0: determines the source cluster size
    :param delta: determines the relative increase in the target cluster size
    """
    if rho0 is None:
        rho0 = 1./n_clusters
    ...
    adjacency, stimulus_segments, text = generate_modular_connections(n_src=mod_pars.N, n_tgt=mod_pars.N,
                                                                      src_layer=0, tgt_layer=1,
                                                                      mod_pars=mod_pars, plot=True, debug=False)
    return adjacency, stimulus_segments, text
